...
def test_no_double_extension_reunites_one_file(tmp_path):
    """Regression (minor 56): a name that already carries .md must not become x.md.md — the bug that
    made a familiar's notebook unfindable, so it founded a 'new' one and minted a false memory."""
    ws = Workshop(tmp_path / "workshop")
    ws.append("first page", artifact="maker-notebook.md")     # name with extension
    ws.append("second page", artifact="maker-notebook")       # same name without
    ws.append("third page", artifact="maker-notebook.md.md")  # even a doubled name collapses
    assert ws.artifacts() == ["maker-notebook.md"]            # ONE file, findable as name.md
    assert len(ws.recent(9, artifact="maker-notebook.md")) == 3


def test_effector_write_to_named_notebook_is_single_extension(tmp_path):
    """The effector must pass the raw target to the workshop, not re-suffix it (the doubling site)."""
    eff = WorldEffector(ww_client=None, session_id="s", identity=_identity(), memory_dir=tmp_path / "m",
                        workshop=Workshop(tmp_path / "workshop"), all_writes_to_workshop=True)
    asyncio.run(eff(Act(kind="write", body="an entry", target="maker-notebook.md")))
    assert (tmp_path / "workshop" / "maker-notebook.md").exists()
    assert not (tmp_path / "workshop" / "maker-notebook.md.md").exists()
