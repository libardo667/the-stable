...
def test_queues_are_pure_event_log_views(tmp_path):
    memory_dir = tmp_path / "memory"
    packets = StimulusPacketQueue(memory_dir / "stimulus_packets.json")
    intents = IntentQueue(memory_dir / "intent_queue.json")

    packets.ensure_file()
    intents.ensure_file()

    # The queues derive their state from the runtime ledger, so an empty ledger
    # reports empty without any json shadow being written for the queues.
    assert packets.all() == []
    assert intents.all() == []
