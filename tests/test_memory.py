...
from src.runtime.memory import derive_memories, memories, record_kept, record_pruned
from src.runtime.pulse import Pulse, route_pulse


def test_pruned_memory_leaves_active_but_stays_recoverable(tmp_path):
    """Soft prune: a tombstone removes a note from active memory but the record persists, and
    re-keeping the note brings it back (the keeper's 'I keep your record' promise stays intact)."""
    record_kept(tmp_path, "the keeper moves to the Netherlands")
    record_kept(tmp_path, "I decided to keep a threshold log")
    assert {m["note"] for m in memories(tmp_path)} == {"the keeper moves to the Netherlands", "I decided to keep a threshold log"}

    record_pruned(tmp_path, "I decided to keep a threshold log")
    active = {m["note"] for m in memories(tmp_path)}
    assert active == {"the keeper moves to the Netherlands"}  # set down — out of active memory
    # but not destroyed: the record still holds both the keep and the tombstone
    raw = (tmp_path / "kept_memory.jsonl").read_text(encoding="utf-8")
    assert raw.count("threshold log") == 2

    record_kept(tmp_path, "I decided to keep a threshold log")  # take it up again
    assert "I decided to keep a threshold log" in {m["note"] for m in memories(tmp_path)}
