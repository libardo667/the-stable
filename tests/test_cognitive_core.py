...
def test_effector_addresses_absent_person_citywide(tmp_path):
    world = _StubWorld(_Scene())
    eff = WorldEffector(ww_client=world, session_id="s1", identity=_identity(), memory_dir=tmp_path, location_hint="Chinatown")
    eff.present = ["Levi"]  # Levi is here; Anika is across the city

    asyncio.run(eff(Act(kind="speak", body="Here's your tea.", target="Levi")))  # co-located → local
    asyncio.run(eff(Act(kind="speak", body="The hum is tighter, Anika.", target="Anika Vance")))  # absent → citywide
