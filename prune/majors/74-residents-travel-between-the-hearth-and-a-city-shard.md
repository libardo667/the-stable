...
- ID: 74-residents-travel-between-the-hearth-and-a-city-shard
- Type: major
- Owner: Levi
...
2. **`src/familiar/city_world.py`** — a lean `CityWorld(client)` wrapping `CityClient`, implementing the
   ...
   need. Adds an honest **`situational_facts()`** for the city (Major 70 style): states the verifiable
...
- [ ] `situational_facts()` for the city states only verifiable facts; no "what to feel" (Major 70).
- [ ] Tests green.
