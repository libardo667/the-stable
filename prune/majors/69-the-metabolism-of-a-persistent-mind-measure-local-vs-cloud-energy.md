...
> low cost. It is a **flagged experiment, not a settled build**, and it depends on Minor 63
...
Minor 47 told the **dollar** story (loop-era ~$355/24d → substrate ~$4/day → local $0
...
- **Connects to:** Minor 63 (the enabling per-pulse mass instrument — hard prerequisite),
  Minor 47 (the dollar baseline this extends into energy), Major 68 (tiered pens — its
  Phase-2 "cost-per-day" claim becomes evidence once energy is measured), Majors 51/60
  (local-pen distillation — the thing that makes more of the roster locally metabolizable),
  Major 63 (the dischargeability dial — where the welfare↔footprint coupling below lands).
...
- **Phase 0 — mass term (Minor 63).** Per-pulse tokens + model + latency + `pen_local` in the
...
- (prereq) Minor 63 across `src/inference/client.py`, `src/runtime/pulse_engine.py`,
...
- [ ] Minor 63 is merged and a real familiar's ledger carries per-pulse mass before any
      energy run begins.
...
- **Risk: laundering hybrid reality as local-first.** Mitigation: every figure is labeled by
  pen locality (Minor 63's `pen_local`); the "14/16 are cloud" confound is stated in the
  writeup, not buried.
...
- **Rollback:** the whole major is additive and offline — drop the plug harness and the
  preregistration; Minor 63's ledger fields stand on their own (they also serve dollar cost
  and Major 68). Nothing in the runtime depends on this experiment existing.
