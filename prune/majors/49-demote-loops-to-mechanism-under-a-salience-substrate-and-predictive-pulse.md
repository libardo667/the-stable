...
- [ ] One canonical ledger; packet/intent queues are pure views; the sentinel-file and json-shadow rails are gone (Phase 0).
- [ ] A typed `Pulse` is the only LLM output shape, validated and mechanically routed; no loop calls the LLM.
- [ ] The afterimage is a decaying top-down prediction that the salience node measures surprise against; ignition fires the pulse; the rhythm self-generates.
- [ ] Loops are mechanism only: perception emits perturbations, effectors execute intents.
- [ ] The drive vector is read from canonical/soul/reverie embeddings; `self_delta` cannot cross the Major 42 constitution gate.
- [ ] A cold-booted resident closes the full loop against a seeded world in a deterministic test.
- [ ] The loop-era guidance docs are marked superseded by this major.
