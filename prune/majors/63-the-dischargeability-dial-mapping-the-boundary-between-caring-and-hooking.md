...
- **Depends on:** Minor 59 (the coercion-ethics protocol) — this experiment deliberately
...
- **Sequencing:** no build during the pilot burn. Venue note: the powered version likely
  belongs in worldweaver's city cohort (peers exist there; the brief's cross-pollination rule —
  a channel validated here goes back to the powered venue); the-stable owns the design, the
  Stage-0 gate, and any small-N isolated pilot.
...
3. **Termination is pre-registered.** Each arm carries a pre-declared stop condition for the
   subject's sake (not only for data quality), per Minor 59.
...
- harness: a controlled-peer stub (responsiveness-scripted, content-honest) — new file under
  `research/harness/`
- worldweaver twin prereg if/when the powered venue is the city (cross-repo, per the
  cross-pollination rule)

## Acceptance Criteria

- [ ] Minor 59 (coercion-ethics protocol) exists and this design passes it, on the record
