...
- ID: 64-measure-the-cost-of-order-blind-affect-escalation-vs-de-escalation
- Type: minor
- Owner: Levi
- Status: **Arm A run 2026-06-12** (see Result); Arm B (behavior) + baseline probe + downstream build still pending
- Risk: low
...
pulse/act divergence against the **matched-window noise floor** (Minor 50) so any divergence
...
measured against the Minor 50 matched-window noise floor — the one arm that costs burn). The
...
- [ ] (If Arm B run) behavior divergence is reported against the Minor 50 matched-window noise
