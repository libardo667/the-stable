...
class WorldEffector:
    ...
    async def _write(self, act: Act) -> dict[str, Any]:
        ...
        if to_workshop:
            ...
            # Pass the raw target; the workshop is the single authority on the filename (it appends
            # the extension only if missing, and collapses a doubled one). Re-suffixing here was the
            # bug that made "maker-notebook.md" → "maker-notebook.md.md" — a notebook a familiar then
            # couldn't find, so it founded a "new" one and minted a false memory (minor 56).
            artifact = "journal.md" if kind in {"journal", "diary", "log", "notes", ""} else kind
