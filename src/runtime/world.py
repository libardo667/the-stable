...
class WorldClient(Protocol):
    ...
    # situational grounding (optional, Major 70). A world reports VERIFIABLE facts about the
    # entity's circumstances; the substrate renders them (identity.render_situational_briefing)
    # into the system prompt, stating what is true and withholding every verdict about what it
    # means. The world supplies switches, not prose, so the one rendering site is the only place
    # a claim is phrased — and the only place to review the fact/verdict line. The recognised keys
    # (all optional; omit what doesn't apply):
    ...
    #                stays undischargeable (no act summons them). See docs/grief-and-coupling.md.
    ...
    # no venue defaults, so a world reports its OWN truth at full resolution and inherits none of another's
    # (a city resident is not told it has a keeper or a workshop; a hearth familiar is not told it has peers
    # or a public world). A key the renderer does not know is logged loudly by the core (drift). A world
    # that does not implement this at all yields no briefing — silence beats a borrowed story. The method
    # is sync: it reads configured switches, not the world.
