...
def _safe_artifact_name(artifact: str) -> str:
    ...
    # Collapse an accidentally doubled extension ("maker-notebook.md.md" -> "maker-notebook.md").
    # A familiar names its notebook with the extension; a caller that re-suffixes would otherwise
    # strand it under a name the familiar can't find on the way back (minor 56 — the false memory).
    for ext in _ALLOWED_EXT:
        while raw.endswith(ext + ext):
            raw = raw[: -len(ext)]
