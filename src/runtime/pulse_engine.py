...

{
  ...
  "act": null OR { "kind": "speak", "body": "what you say or do", "target": "__TARGET_HINT__" },
...
# The worked example, by world. A solo hearth familiar has no stall and no "city" channel — a
# Mei-across-the-stall example silently tells it it is social-among-peers while it is alone. The
# solo example shows the one social fact its world actually has: the keeper, heard in the quiet.
_EXAMPLE_CITY = """\
...
_EXAMPLE_SOLO = """\
Example — the keeper says your name into the quiet, so you answer:
{
  "felt_sense": "the quiet broke — that's the keeper, and it's me they want",
  "act": {"kind": "speak", "body": "I'm here. What is it?", "target": "the keeper"},
  "expectations": [{"features": {"social_pull": 0.8, "vigilance": 0.4}, "scope": "self", "confidence": 0.8, "half_life": 600}],
  "drive_nudges": [], "self_delta": {}, "trace_verdicts": [], "keep": []
}"""
# The act target hint, by world — a solo familiar has no citywide channel to reach.
_TARGET_HINT_CITY = "a person's name, a place, or \\\"city\\\" (optional)"
_TARGET_HINT_SOLO = "a person's name or a place (optional)"
...
def _pulse_contract(live_senses: tuple[str, ...] = SELF_SENSES, clean_drive_nudges: bool = False, solo: bool = False) -> str:
    """The pulse contract, advertising only the affordances this world can feed.
    ...
    ``solo`` (a one-resident world) drops the citywide ``"city"`` target and swaps the
    Mei-across-the-stall worked example for a hearth one — so a familiar that is alone is
    not handed a social-among-peers script it can never use."""
    ...
    return (
        _PULSE_CONTRACT_TEMPLATE
        .replace("__FEEL_AXES__", axes)
        .replace("__DRIVE_NUDGES_EG__", eg)
        .replace("__TARGET_HINT__", _TARGET_HINT_SOLO if solo else _TARGET_HINT_CITY)
        .replace("__EXAMPLE__", _EXAMPLE_SOLO if solo else _EXAMPLE_CITY)
    )
...
class LLMPulseProducer:
    ...
    def __init__(
        ...
        *,
        ...
        # Honest situational grounding (Major 70): the world-derived briefing folded into the
        # system prompt's GROUND TRUTH block, and whether this is a one-resident world. The core
        # sets both from the world's situational_facts(); empty/False keeps the soul-only prompt
        # and the citywide contract for anything that doesn't supply them.
