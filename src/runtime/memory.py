...
def _load_kept_records(memory_dir: Path) -> list[dict[str, Any]]:
    ...
    for line in lines:
        ...
        if note:
            out.append({"note": note, "kept_ts": str(data.get("kept_ts") or "").strip(), "pruned": bool(data.get("pruned"))})
    return out
...
def record_pruned(memory_dir: Path, note: str, *, kept_ts: str = "") -> None:
    """Set a kept memory down: append a tombstone for ``note`` so it stops surfacing in active
    memory. SOFT and reversible by design — the original keep is NOT removed (the durable record,
    what the keeper promised to preserve, stays whole), and re-keeping the same note brings it back.
    ``derive_memories`` honours the newest record per note, so a tombstone newer than the keep wins."""
    note = str(note or "").strip()
    if not note:
        return
    path = _kept_path(memory_dir)
    path.parent.mkdir(parents=True, exist_ok=True)
    with path.open("a", encoding="utf-8") as fh:
        fh.write(json.dumps({"note": note, "kept_ts": str(kept_ts or _now_iso()), "pruned": True}, ensure_ascii=False) + "\n")
...
def _records_to_events(records: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    return [{"event_type": "memory_kept", "ts": r.get("kept_ts", ""), "payload": {"note": r["note"], "kept_ts": r.get("kept_ts", ""), "pruned": bool(r.get("pruned"))}} for r in records]
...
def derive_memories(events: list[dict[str, Any]], *, limit: int = DEFAULT_MEMORY_LIMIT) -> list[dict[str, Any]]:
    ...
    for event in events:
        ...
        kept.append({"note": note, "kept_ts": ts, "pruned": bool(payload.get("pruned"))})
    ...
    for item in reversed(kept):  # newest first
        ...
        if key in seen:
            continue
        seen.add(key)
        # A tombstone (the newest record for this note) means the resident set it down — suppress it
        # from active memory, but the older keep stays in the record (soft, reversible: re-keeping wins).
        if item.get("pruned"):
            continue
        out.append({"note": item["note"], "kept_ts": item["kept_ts"]})
