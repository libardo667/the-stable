...
_BINARY_SNIFF = 2048
_MAX_MEDIA_BYTES = 20_000_000  # a single image/PDF read ceiling — perception, not bulk transfer
...
class FileScope:
    ...
    def _resolve(self, path: Any) -> tuple[Path | None, Path | None]:
        ...
        # A bare "<root-name>" or "<root-name>/" resolves to the root directory itself,
        # so an agent can open a root folder by name to list it (not only deep-link a
        # file inside it). Without this, "read received-from-client/" returns not_found
        # instead of a folder listing, and a familiar can get stuck unable to browse in.
    ...
    def read(self, path: Any, *, offset: int = 0, max_bytes: int = _MAX_READ_BYTES) -> dict[str, Any]:
        """Read a window of a file's text — ``data[offset : offset+max_bytes]`` — only if it is
        inside a root and not ignored. ``offset`` is what makes pagination possible: a big file is
        read a page at a time. ``bytes_total`` is the whole file's size, ``offset`` the window
        start, and ``truncated`` means there is more text *after* this window (another page)."""
        ...
        return {"ok": True, "path": self._display(resolved, root), "root": root.name, "content": text,
                "offset": offset, "bytes_total": len(data), "truncated": offset + len(window) < len(data)}

    def read_media(self, path: Any) -> dict[str, Any]:
        """Read the *raw bytes* of an in-scope **image or PDF** — the one exception to the binary
        refusal in ``read()``. The two structural guards are identical (resolve inside a root,
        respect ignore + the secret default-deny); only the binary check is relaxed, and only for
        recognised visual types — anything else still refuses (``not_visual``). The caller turns
        these bytes into a perception via ``visual.to_perception`` (this method never decides what a
        mind can *see*; it only clears the bytes). ``kind`` is ``"image"`` or ``"pdf"``."""
        from . import visual  # local import keeps file_scope importable without the pdfium dep
        ...
        if not resolved.is_file():
            return {"ok": False, "reason": "not_a_file"}
        if self._ignored(resolved, root):
            return {"ok": False, "reason": "ignored"}  # secret default-deny still applies, before any bytes are read
        try:
        ...
        return {"ok": True, "path": self._display(resolved, root), "root": root.name, "kind": kind, "data": data, "bytes_total": len(data)}
    ...
    def find_by_name(self, name: Any, *, limit: int = 4, max_scan: int = 3000) -> list[str]:
        """Qualified display paths of non-ignored files whose basename matches ``name``
        (case-insensitive), across all roots. Best-effort and bounded — for a 'did you
        mean' recovery when a read fails because the agent guessed the wrong root or gave
        a bare name (the resolver only matches a path under a single root, so a wrong-root
        prefix won't fall through to where the file actually lives). Never reaches outside
        a root or past an ignore rule, so it cannot suggest a path the agent may not read."""
        want = os.path.basename(str(name or "").strip().rstrip("/")).lower()
        if not want:
            return []
        out: list[str] = []
        scanned = 0
        for root in self.roots:
            stack = [root]
            while stack and len(out) < limit and scanned < max_scan:
                d = stack.pop()
                try:
                ...
                for child in children:
                    ...
                    if child.is_dir():
                        stack.append(child)
                    elif child.name.lower() == want:
                        out.append(self._display(child, root))
                        if len(out) >= limit:
                            break
        return out
