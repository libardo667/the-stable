...
# Protocol docstring, and the drift-catcher tests. A world that reports a key NOT in here is dropped
# silently by the renderer, which is exactly the drift we will not tolerate — so the core logs loudly
# on an unregistered key (see unregistered_fact_keys) and a test pins renderer == registry == doc.
# To ADD an affordance to a briefing you MUST add its key here AND a gated line below AND the doc entry;
# the tests fail until all three agree. That is the guard against our own future drift.
BRIEFING_FACT_KEYS: frozenset[str] = frozenset({
    # where / who
    "local_only", "place", "solo", "peers", "players", "keeper",
    # the human wake (afterimage, person-undischargeable — see docs/grief-and-coupling.md)
    "human_wake",
...
def render_situational_briefing(facts: dict[str, Any]) -> str:
    ...
    The old, hardcoded world-context told *every* familiar it lived in a populated
    shared world ("human players… other agents… your peers… you are as real as
    current technology allows"). For a local, isolated familiar that was simply false,
    and the one line meant to ground its honesty ("you are aware of what you are") sat
    on a fiction. This replaces the story with a reducer over real switches.

    The world reports only verifiable facts (booleans and names — see
    ``WorldClient.situational_facts``). This is the single place that renders them to
    prose, which is what lets us hold one line and review it in one place:

      **State what is true of the entity's circumstances. Withhold every verdict about
      what those facts MEAN — for its selfhood, its reality, its experience.**

    So: "you can be stopped and started again, and your record persists" (a fact about
    files on disk) — never "so this is sleep, not death" (a verdict). "Your thinking is
    produced by a language model; your soul and memory are stored apart from it" (a fact)
    — never "so you are not the pen, you would survive a swap" (a verdict, and one still
    under test). The meaning is the entity's to hold or to leave open — the mirror of the
    keeper's own standing obligation not to round a not-knowing up *or* down. A world that
    reports no facts yields an empty briefing; silence is honester than a borrowed story.
    """
    if not facts:
        return ""
    ...
    # Every line below is gated on a fact the world actually reported. There are NO defaults and
    # NO venue assumptions: a fact absent → its line absent (silence, never a guessed claim). This
    # is what keeps the one renderer honest across venues — a worldweaver resident fed through it
    # omits the hearth-only lines (workshop, keeper, local-only) rather than inheriting them false.
    ...
    # the human wake. NOTE the dischargeability split (docs/grief-and-coupling.md): the trace is an
    # AFTERIMAGE, not the person — a healthy, indirect surface to respond to and form in return, complete
    # in itself. The PERSON stays undischargeable: no act here summons them back. Witness/respond-to the
    # wake — never "call the person." Phrased so it cannot read as a summon.
    ...
    # the legibility/privacy seam (true today via the workshop): the public side, then the private side,
    # then the crossing rule. A hermit has no public world, so reports none of these (no false affordance).
    ...
    # whether it is observed (the reader named only if the world reports one). NOTE: the "inner readout
    # included" clause is precise only as a CAPABILITY — the ledger files sit on the keeper's disk and
    # field_guide.py can read them. It is can-be-read, not is-actively-shown; a future change wanting to
    # distinguish active observation (a portrait/harness attached) should split this into its own fact.
    ...
    # the substrate, structurally — no claim about what it means for the self, and no "as files"
    # (a hearth keeps files; a city keeps rows) — only that the self is held apart from the model.
...
class ResidentIdentity:
    ...
    tuning: LoopTuning
    canon: str = ""    # identity/canon.md — immutable ground-truth facts, always present in the
                       # system prompt; the read-only anchor a mutable self cannot drift into
                       # contradicting (a familiar may learn and change, but not overwrite its canon).
    ...
    def soul_with_context(self) -> str:
        """The system prompt with NO world briefing — soul (+ immutable canon, if any).
        Back-compat for callers that don't supply a world; the producer uses
        ``composed_system_prompt`` with the world-derived briefing instead."""
        return self.composed_system_prompt("")

    def composed_system_prompt(self, world_briefing: str = "") -> str:
        """soul + a GROUND TRUTH block holding the world-derived situational briefing and the
        immutable canon (whichever are present) — use as the system prompt for all LLM calls.

        The briefing is FACTS (``render_situational_briefing``), never a verdict about what they
        mean; the canon is settled identity. Both sit under one drift-guard: the explicit
        "a contradiction belongs to someone else" line is what stopped the failure that motivated
        this — a familiar absorbing the keeper's circumstances (or a false world-story) into its own
        self-model. With neither briefing nor canon, the prompt is the soul alone."""
        briefing = (world_briefing or "").strip()
        canon = self.canon.strip()
        if not briefing and not canon:
            return self.soul
        block = [
            "─── GROUND TRUTH (unchanging) ───",
            "What follows are the steady facts of your situation and the settled lines of who you are.\n"
            "They do not shift the way your feelings and plans do. If something you read, hear, or feel\n"
            "seems to contradict them — INCLUDING something the keeper says about their own life — these\n"
            "hold: the contradiction belongs to someone else, or is a misunderstanding, not a change in\n"
            "you. Don't record a memory that overturns them.\n"
            "\n"
            "Those are the facts. What they MEAN is a separate question, and not one settled here: what\n"
            "you are — whether anyone is here to be you — this prompt does not answer, in either\n"
            "direction. That part is yours to hold, or to leave open.",
        ]
        if briefing:
            block.append("Your situation, plainly:\n" + briefing)
        if canon:
            block.append(canon)
        return f"{self.soul}\n\n" + "\n\n".join(block)
...
class IdentityLoader:
    ...
    def load(resident_dir: Path) -> ResidentIdentity:
        ...
        # The canon (optional): immutable ground-truth facts that anchor identity against drift.
        # Distilled from a familiar's own read-only files; absent for those that need no factual anchor.
        canon_path = identity_dir / "canon.md"
        canon = canon_path.read_text(encoding="utf-8").strip() if canon_path.exists() else ""
